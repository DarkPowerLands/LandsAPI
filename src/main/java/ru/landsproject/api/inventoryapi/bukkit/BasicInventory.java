...
public class BasicInventory {
    ...
    private BasicInventory parent;

    private List<InventoryListener<? extends Event>> listeners;
    private InventoryManager manager;

    private BasicInventory(InventoryManager manager) {
        this.manager = manager;
    }

    public Inventory open(Player player) { return open(player, 0); }
    public Inventory open(Player player, int page) {
        Optional<BasicInventory> oldInv = this.manager.getInventory(player);
    ...
    public Optional<BasicInventory> getParent() { return Optional.ofNullable(parent); }
    ...
    public static final class Builder {
        ...
        public Builder parent(BasicInventory parent) {
            this.parent = parent;
            return this;
        }
        ...
        public BasicInventory build() {
            if(this.provider == null)
                throw new IllegalStateException("The provider of the BasicInventory.Builder must be set.");
            if(this.manager == null)
                throw new IllegalStateException("The manager of the BasicInventory.Builder must be set.");
            InventoryManager manager = this.manager;

            BasicInventory inv = new BasicInventory(manager);
            inv.id = this.id;
